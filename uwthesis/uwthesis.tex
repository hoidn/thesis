...
\begin{figure}[h] 
\caption{The atlas of high-energy density physics}% TODO: (cite)}
\label{fig:atlas}
\centering
...
%PENELOPE uses the static field approximation, which is accurate above energies of a few hundred eV but introduces a considerable error to the elastic DCS at and below 1 keV.\cite{mott1965theory, salvat2008penelope} 
...
%\end{multline}
%We note that, although PENELOPE accounts for the effect of the finite nuclear size on the elastic DCS, we simply treat the nucleus as a point source as this is sufficient for electrons with energy below 1 MeV.\cite{salvat2008penelope} 
...
\end{equation}
% TODO check if egerton has cites for wentzel and lenz
...
More detailed treatment shows that, in the case of a helical undulator, only the fundamental mode has strong on-axis emission.% TODO  (cites)
